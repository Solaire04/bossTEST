...
LD_LIBRARY_PATH="/opt/ASI_linux_mac_SDK_V1.38/lib/x64:$LD_LIBRARY_PATH" C_INCLUDE_PATH="/opt/ASI_linux_mac_SDK_V1.38/include:$C_INCLUDE_PATH" exec wish "$0" "$@"
...
### 2025.06.29: GPIB generic interface, macro execution, tooltips
### 2025.07.12: spectral calibration, loadable reference data
###
set APPNAME     [lindex [file split [info script]] end]
set PLATFORM    [lindex [array get tcl_platform os] 1]
set VERSION     2025.07.22

### at runtime, can set these flags, if passed as run-time parameters
set DEBUG 0
set NOGPIB 0
foreach arg $argv {
  if {"$arg"=="DEBUG"}  { set DEBUG 1 }
  if {"$arg"=="NOGPIB"} { set NOGPIB 1 }
  }
...
set MIN_W       1080
set MIN_H       720
set TABS        {Settings Camera Spectrum Calibrate GPIB}
...
### top left corner of the window frame; program still runs if this icon is missing
set iconfile /usr/local/share/icons/hicolor/64x64/apps/boss.png
...
if { [catch {package require Img}] } {
  ...
  set imgVersion [package require img::raw]
  if {[package vsatisfies $imgVersion 2]} {set raw2 1} else {set raw2 0}
  if {$DEBUG} {puts stderr "DEBUG: Img library version is $imgVersion; raw2 = $raw2"}
  }
...
if {$NOGPIB} {
  if {$DEBUG} { puts stderr "DEBUG: NOGPIB requested, disabling all GPIB funcions" }
  }\
else {
...
### hardware access API through the manufacturer's C library; added to LD_LIBRARY_PATH at run-time
set LD_LIBRARY_PATH [string trimright $::env(LD_LIBRARY_PATH) ":"]
set C_INCLUDE_PATH  [string trimright $::env(C_INCLUDE_PATH) ":"]
if {$DEBUG} {puts stderr "DEBUG: -L$LD_LIBRARY_PATH -I$C_INCLUDE_PATH"}

critcl::cheaders -I$C_INCLUDE_PATH
critcl::clibraries -L$LD_LIBRARY_PATH
...
critcl::cproc ASIGetCameraInfo {int i} object0 {
  ...
  char str[256], *vformats, *bins, dum[64];
  int j, n=0;
  asprintf(&vformats,"");
  for (j=0; j<8; j++) {
    if (cameraInfo.SupportedVideoFormat[j] == ASI_IMG_END ) { break; };
    strncpy(dum, vformats, 64);
    n = asprintf(&vformats,"%s %d",dum,cameraInfo.SupportedVideoFormat[j]);
    if (n < 0) { fprintf(stderr,"ERROR: in asprintf allocation\n");}
    }
  asprintf(&bins,"");
  for (j=0; j<16; j++) {
    if (cameraInfo.SupportedBins[j] == 0 ) { break; };
    strncpy(dum, bins, 64);
    n = asprintf(&bins,"%s %d",dum,cameraInfo.SupportedBins[j]);
    if (n < 0) { fprintf(stderr,"ERROR: in asprintf allocation\n");}
    }
  sprintf(str,"id %d name {%s} width %ld height %ld bitdepth %d pixelsize %.3fnm colour %s cooler %s shutter %s usb3 %s vformats { %s } bins { %s }", 
    cameraInfo.CameraID, cameraInfo.Name, 
    cameraInfo.MaxWidth, cameraInfo.MaxHeight, 
    cameraInfo.BitDepth, cameraInfo.PixelSize, 
...
proc LoadSettings { interactive fname } {
  global DEBUG settings WorkDir targetTemp expTime expUnit
  if ($interactive) {
...
proc saveFile { fname } {
  global DEBUG raw2 ::x ::y WorkDir cameraInfo expTime expUnit imgImg ROI ch
  ...
  switch $ftype {
    raw {
      if {$raw2} { set AOI [$imgImg data -format raw -grayscale] }\
      else {       set AOI [base64::decode [$imgImg data -format raw -grayscale]] }
      fconfigure $fp -translation binary -encoding binary
      puts -nonewline $fp $AOI
      close $fp
      unset AOI
    ...
    cal {
      for {set j 0} {$j < [::calW length]} {incr j} { if {"$ch($j)"!=""} {puts $fp "$ch($j)\t$::calW($j)"} }
      close $fp
      setStatus "ok" "Saved calibration to $filename"
      }
    default {
      close $fp
...
proc loadFile { fname } {
  global DEBUG WorkDir imgImg imgData RAW_IMG_TYPE  ROI ch ncal
  set filename [string tolower [file rootname $fname]]
  set ftype [string trimleft [string tolower [file extension $fname]] "."]
  set file_types {}
  lappend file_types "\{ $ftype \} \{.$ftype [string toupper .$ftype] \}"
  ...
  if {$DEBUG} { puts stderr "DEBUG: filename is \"$filename\" ftype \"$ftype\"\nDEBUG: $file_types" } 
  if { "$filename" == "" } { 
    set filename [tk_getOpenFile -filetypes $file_types -initialdir $WorkDir]
    if { "$filename" == "" } { setStatus "warning" "Load operation cancelled by user"; return }
    }\
  else {
    set filename ${fname}
  ...
  switch $ftype {
    ...
    png {
      ...
      if {[catch {$imgImg read $filename -format $ftype}]} {
        setStatus "error" "Error opening $ftype image file \"$filename\", check permissions & format"
        return
        }
      adjustROI
      }
    cal {
      if {[catch {set fp [open $filename "r"]}]} {
        setStatus "error" "Error opening $ftype calibration file \"$filename\", check permissions"
        return
        }
      set ncal 0
      while {[set cw [gets $fp]]!=""} {
	set c_in [::calW search [lindex $cw 1]] 
	set ch($c_in) [lindex $cw 0]
	incr ncal
        }   
      close $fp
      }
   ref {    
      if {[catch {set fp [open $filename "r"]}]} {
        setStatus "error" "Error opening $ftype reference file \"$filename\", check permissions"
        return
        }
      #if { ![catch {table info .ts.cb.l .ts.cb.l.info}] } 
      table forget .ts.cb.l.actions .ts.cb.l.info
      if {[set nRef [::calW length]]>0} {
        for {set i 2; set j 0} {$j<$nRef} {incr i; incr j} {
          table forget .ts.cb.l.n$i .ts.cb.l.s$i .ts.cb.l.w$i .ts.cb.l.a$i .ts.cb.l.c$i
          destroy .ts.cb.l.n$i .ts.cb.l.s$i .ts.cb.l.w$i .ts.cb.l.a$i .ts.cb.l.c$i
          }
        }
      vector destroy ::calW ::calA
      vector create  ::calW ::calA
      while {[set line [gets $fp]]!=""} {
         set line [string trim $line]
         set i [string first "#" $line]
         if {$i>0} {set data [string range $line 0 $i-1]} else {set data $line}
         if {$i!=0} { 
           ::calA append [lindex $data 0]
           ::calW append [lindex $data 1]
           }
         }
       close $fp
      }
...
proc displaySpectrum {x1 y1 x2 y2} {
  global DEBUG raw2 ::x ::y imgImg imgW imgH h_size v_size slope yint
  set h_size [expr $x2 - $x1]
  set v_size [expr $y2 - $y1]
  set imgH [image height $imgImg]
  set imgW [image width $imgImg]
  ...
  #set AOI ""
  if {$raw2} { set AOI [$imgImg data -format raw -grayscale -from $x1 $y1 $x2 $y2] }\
  else {       set AOI [base64::decode [$imgImg data -format raw -grayscale -from $x1 $y1 $x2 $y2]] }
...
set imgType 2
set imgW [expr max($MIN_W,[winfo screenwidth . ]-100)]
set imgH [expr max($MIN_H,[winfo screenheight . ]-100)]
...
set ncal 0
#*******************************************************************************
# Plasma lines of a He-Ne laser, from https://books.google.ca/books?id=f_sJ8CGfetYC&pg=PA101
#-------------------------------------------------------------------------------
vector create ::calW ::calA
::calW set { 638.18 640.05 640.87 642.06 644.39 650.60 653.29 \
  660.02 664.43 665.48 667.05 668.20 672.23 674.38 }
::calA set { 15 19 3 1 4 18 3 19 1 1.5 2 50 28 2 }
#*******************************************************************************
...
if { [catch {open $iconfile} icp] } {
   if ($DEBUG) {puts stderr "DEBUG: icon file missing/unable to read $iconfile"}
   }\
else {
   close $icp
   wm iconphoto . [image create photo -file $iconfile]
   }
...
# Settings tab, report/change all control parameters
...
foreach paramName [dict keys $settings] {
  global $paramName ${paramName}_h
  if {"$paramName"=="wmG"} {continue}; # skip over the wm geometry, keep it internal
...
.ts tab configure Camera -window .ts.lv -expand 1 -fill both

# The Spectrum tab is for extracting a spectrum from ROI
...
button .ts.sp.tbar.cal -text "Calibrate \u259e" -state normal -command {blt::CreateTearoff .ts Calibrate}
...
pack .ts.sp.tbar.file .ts.sp.tbar.lroi .ts.sp.tbar.roi .ts.sp.tbar.reset .ts.sp.tbar.cal -side left -fill x
...
## note inside bind, % has meaning, needs escaping as %%
bind .ts.sp.g <2> {
  global sx yint slope ch ncal
  set ch($ncal) [format "%%.2f" [expr ($sx-$yint)/$slope]]
  }

.ts tab configure Spectrum -window .ts.sp -expand 1 -fill both

frame .ts.cb
frame .ts.cb.l
label .ts.cb.l.title -text "Calibration using a reference spectrum" -font TkHeadingFont
label .ts.cb.l.n -text "Peak #" 
label .ts.cb.l.w -text "\u03BB, nm"
label .ts.cb.l.a -text "Strength"
label .ts.cb.l.c -text "Pixel #"
table .ts.cb.l 0,0 .ts.cb.l.title -cspan 5
table .ts.cb.l 1,0 .ts.cb.l.n -cspan 2 1,2 .ts.cb.l.w 1,3 .ts.cb.l.a 1,4 .ts.cb.l.c

frame .ts.cb.l.actions
button .ts.cb.l.actions.save  -text "Save .cal" -width 11 -bg LightGreen -command {saveFile ".cal"}
button .ts.cb.l.actions.load  -text "Load .cal" -width 11 -bg LightGreen -command {loadFile ".cal"}
button .ts.cb.l.actions.sim   -text "Simulate"  -width 11 -bg LightGreen -command {simulateCal}
button .ts.cb.l.actions.fit   -text "Fit"       -width 11 -bg LightGreen -command {fitCal}
button .ts.cb.l.actions.apply -text "Apply"     -width 11 -bg LightGreen -command {applyCal} -state disabled
button .ts.cb.l.actions.undo  -text "Undo"      -width 11 -bg LightGreen -command {undoCal}
button .ts.cb.l.actions.clear -text "Clear"     -width 11 -bg LightGreen -command {clearCal}
button .ts.cb.l.actions.ref   -text "Ref"       -width 11 -bg LightGreen -command {clearCal; loadFile ".ref"; fillCalTable}

label .ts.cb.l.info -font TkHeadingFont \
    -text "Select a peak, find it in the Spectrum tab,\nthen centre-click its midpoint to record the\npeak's pixel #. Fit needs 3+ points."

proc fillCalTable {} {
  global DEBUG ncal ch
  if {$DEBUG} {puts "DEBUG: reference wavelenghts are: $::calW(:)"}
  set i 2
  for {set j 0} {$j<[::calW length]} {incr j; incr i} {
    radiobutton .ts.cb.l.s$i -variable ncal -value $j
    label .ts.cb.l.n$i -text [expr $j+1] -anchor e -width 3
    label .ts.cb.l.w$i -text [format "%.2f" $::calW($j)] -width 7
    label .ts.cb.l.a$i -text [format "%.1f" $::calA($j)] -width 6
    entry .ts.cb.l.c$i -textvar ch($j) -width 7
    table .ts.cb.l $i,0 .ts.cb.l.n$i $i,1 .ts.cb.l.s$i $i,2 .ts.cb.l.w$i $i,3 .ts.cb.l.a$i $i,4 .ts.cb.l.c$i
    }
  table .ts.cb.l.actions 0,0 .ts.cb.l.actions.sim  0,1 .ts.cb.l.actions.fit 0,2 .ts.cb.l.actions.clear
  table .ts.cb.l.actions 1,0 .ts.cb.l.actions.load                          1,2 .ts.cb.l.actions.apply
  table .ts.cb.l.actions 2,0 .ts.cb.l.actions.save 2,1 .ts.cb.l.actions.ref 2,2 .ts.cb.l.actions.undo
  incr i 
  table .ts.cb.l $i,0 .ts.cb.l.actions -cspan 5
  incr i
  table .ts.cb.l $i,0 .ts.cb.l.info -cspan 5
  event generate .ts.cb.l <Configure>
  }

fillCalTable

frame .ts.cb.r
#table .ts.cb 0,5 .ts.cb.r -rspan [expr $i+1] -fill y
pack [graph .ts.cb.r.g -width [expr max(720,[winfo screenwidth .ts.cb.r])] -bd 2 -relief groove] -fill y -expand 1
.ts.cb.r.g element create points -pixels 5 -linewidth 0
.ts.cb.r.g element create line -pixels 1 -linewidth 1 -color green
.ts.cb.r.g legend configure -hide 1
.ts.cb.r.g element create peaks

proc clearCal {} {; # erase cal table
  global ncal ch
  for {set j 0} {$j<[::calW length]} {incr j} {set ch($j) ""}
  set ncal 0
  .ts.cb.l.actions.apply configure -state disabled
  }

proc fitCal {} {; # linear fit for x-axis calibration
  global DEBUG ncal ch slope yint
  newVector ::cx "" 
  newVector ::cy ""
  newVector ::lx "" 
  newVector ::ly ""
  for {set j 0} {$j<[::calW length]} {incr j} {
    if {$ch($j)!=""} {cx append $ch($j); cy append $::calW($j)}
    }
  if {[cx length]<2} {
    setStatus "warning" "Need more than one point to fit a straight line"
    return
    }
  .ts.cb.r.g element configure peaks -hide 1
  set xm $::cx(mean); set ym $::cy(mean); set xsum 0.0; set ysum 0.0
  for {set j 0} {$j<[cx length]} {incr j} {
    set dx [expr {$::cx($j) - $xm}] 
    set dy [expr {$::cy($j) - $ym}]
    set xsum [expr {$xsum + $dx * $dy}]
    set ysum [expr {$ysum + $dx * $dx}]
    }
  set slope [expr {$xsum / $ysum}]
  set yint [expr {$ym - $slope * $xm}]
  set xf $::cx(max); set yf [expr {$yint + $slope*$xf}]; # slope,yint from linear fit
  ::lx set { 0 $xf }; ::ly set { $yint $yf }
  .ts.cb.r.g axis configure y -min $::cy(min) -max $::cy(max) -title "wavelength \u03BB, nm"
  .ts.cb.r.g axis configure x -min $::cx(min) -max $::cx(max) -title "pixel #"
  .ts.cb.r.g element configure points -xdata ::cx -ydata ::cy -hide 0; # draw points
  .ts.cb.r.g element configure line -xdata ::lx -ydata ::ly -hide 0; # draw line
  event generate .ts.cb.r.g <Configure>
  .ts.cb.l.actions.apply configure -state normal
  }
 
proc undoCal {} {; # revert back to pixel #
  global slope yint ROI
  set slope 1.0; set yint 0.0
  ::x0 dup ::x
  .ts.sp.g axis configure x -title "pixel #"
  updateXaxis
  displaySpectrum {*}$ROI
  }
...
proc applyCal {} {
  global slope yint ROI
  ::x expr { $slope*::x0+$yint }
  .ts.sp.g axis configure x -title "wavelength \u03BB, nm"
  updateXaxis
  displaySpectrum {*}$ROI
  }
  
proc updateXaxis {} {
  .ts.sp.g element configure spectrum -xdata ::x 
  .ts.sp.g axis configure x -min $::x(min) -max $::x(max)
  .ts.sp.g marker configure bg -coords "$::x(min) $::y(min) $::x(max) $::y(max)"
  set coords [.ts.sp.g marker cget _top -coords]
  lset coords 0 $::x(min); lset coords 2 $::x(max)
  .ts.sp.g marker configure _top -coords $coords
  set coords [.ts.sp.g marker cget _bot -coords]
  lset coords 0 $::x(min); lset coords 2 $::x(max)
  .ts.sp.g marker configure _bot -coords $coords
  }

proc simulateCal {} {
  global DEBUG
  set x_min [expr 10*int($::calW(min)/10)]
  set x_max [expr 10*int($::calW(max)/10+1)]
  newVector ::He_x ""; ::He_x seq $x_min $x_max [expr ($x_max - $x_min)/1500.]
  set s2 [expr ($x_max - $x_min)/500.]
  newVector ::He_y ""; ::He_y expr { 0.0*::He_x }
  newVector ::wx ""
  foreach m [.ts.cb.r.g marker names] {.ts.cb.r.g marker delete $m}
  for {set j 0} {$j<[::calW length]} {incr j} {
    ::wx expr { ($::calW($j)- ::He_x)/$s2 }
    ::He_y expr { ::He_y+$::calA($j)/(1+::wx^2) }
    # add labels above each peak
    set xl $::calW($j)
    set yl [expr 0.05*$::calA(max)+$::calA($j)]
    .ts.cb.r.g marker create text -text [format "%d" [expr $j+1]] -coords { $xl $yl } -fg blue
    }
  .ts.cb.r.g element configure points -hide 1
  .ts.cb.r.g element configure line -hide 1
  .ts.cb.r.g axis configure x -title "\u03BB (nm)" -min $x_min -max $x_max
  set yl [expr 1.1*$::calA(max)]
  .ts.cb.r.g axis configure y -title "" -max $yl -min 0
  set yl [expr $::calA(max)]
  .ts.cb.r.g marker create line -linewidth 3 -outline red -coords { 632.8 0 632.8 $yl }
  .ts.cb.r.g marker create line -linewidth 3 -outline green -coords { 532. 0 532. $yl }
  set yl [expr 1.05*$::calA(max)]
  .ts.cb.r.g marker create text -text "632.80" -coords { 632.8 $yl } -fg red
  .ts.cb.r.g marker create text -text "532.0" -coords { 532. $yl } -fg green
  .ts.cb.r.g element configure peaks -xdata ::He_x -ydata ::He_y -pixels 1 -color black -hide 0
  }
simulateCal

event generate .ts.cb.r.g <Configure>

pack .ts.cb.l .ts.cb.r -side left -fill y -expand 1

.ts tab configure Calibrate -window .ts.cb -anchor n

#-----------------------------------------------------------------------------

# GPIB communications with the spectrometer
...
proc sendGPIB {cmd} {
  ...
  if {[info exists gpib_dev]} {
    ...
    # if the sent command contained "?", expect a reply
      ...
      if {[catch {$gpib_dev read} Rxdata]} {set Rxdata ""; setStatus "warning" "No reply from $gpib_dev"}
    #  }
    }\
...
proc connectGPIB { } {
  ...
  if {[info exists gpib_dev]} {
    ...
    if { [catch {set gpib_dev [gpib_device "GPIB$gpib_board:$dev_addr" -board $gpib_board -address $dev_addr -trimright \n -readymask 16]}] } {
      setStatus "error" "GPIB$gpib_board:$dev_addr not found"
      return
      }
    if {[catch {set dev_id [$gpib_dev cmd_read *IDN?]}]} { set dev_id "unknown device" }
...
if {$NOGPIB} {.ts tab configure GPIB -hidden 1}
...
tooltip::tooltip .ts.sp.tbar.cal "Tearoff the Calibrate tab into a separate window from this Spectrum tab"
...
tooltip::tooltip .ts.cb.l.actions.sim "Simulate a spectrum using current reference wavelengths and strengths"
tooltip::tooltip .ts.cb.l.actions.fit "Fit a straight line to wavelength vs pixel to establish calibration"
tooltip::tooltip .ts.cb.l.actions.apply "Fit first, then apply that calibration to the Spectrum tab"
tooltip::tooltip .ts.cb.l.actions.clear "Clear all pixel entries; changing mirror/grating requires recalibration"
tooltip::tooltip .ts.cb.l.actions.load "Load previously saved wavelength/pixel pairs from a .cal file"
tooltip::tooltip .ts.cb.l.actions.save "Save the current set of wavelength/pixel pairs in a .cal file"
tooltip::tooltip .ts.cb.l.actions.undo "Revert to using camera's pixel index on the x-axis"
tooltip::tooltip .ts.cb.l.actions.ref "Load a different set of reference wavelengths and strengths. Defaults to He-Ne plasma"
tooltip::tooltip .ts.gpib.tbar.port "GPIB \"card\" or \"interface\" address, there could be several"
tooltip::tooltip .ts.gpib.tbar.addr "GPIB \"device\" address on the interface"
tooltip::tooltip .ts.gpib.tbar.null "Some GPIB devices may require a NULL terminator instead of EOL"
...
.ts tab select Camera

findAllCameras
set nCameras [llength $listOfCameras]
if {$DEBUG} { puts stderr "DEBUG: found $nCameras $listOfCameras" }
if { $nCameras >= 1 } {
  .ts.lv.tbar.vdev configure -textvariable cameraNow -values $listOfCameras
  setStatus "warning" "Found $nCameras cameras, selecting the first one"
  set cameraId 0
  set cameraNow [lindex $listOfCameras $cameraId]
  } \
else {
